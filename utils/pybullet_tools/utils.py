...
PR2_CAMERA_MATRIX = get_camera_matrix(
    width=640, height=480, fx=572.55, fy=572.5)
...
def attach_viewcone(camera, depth=MAX_VISUAL_DISTANCE,
...
def create_obj(path, scale=1., mass=STATIC_MASS, color=(0.5, 0.5, 0.5, 1), allow_collision=True):
...
def rot_mat_y(d):
    r = np.pi * d / 180
    return np.array([[np.cos(r), 0, np.sin(r), 0], [0, 1, 0], [-np.sin(r), 0, np.cos(r)]])
...
def rot_mat_n(d, n0):
    ...
    return np.array([[nxx * ic + c, nxy * ic - nz * s, nxz * ic + ny * s],
...
def get_raytest_scatter2(target_body, robot):
    ...
    f_base_ori_jj = math.acos(
        np.dot(base_ori_mat.T[2], rt_vector_uni))  # 夹角 angle between vectorR2T and vectorZaxis of robot base

    """in frame target_obj"""
    tr_vector_t = np.array([np.dot(x_axis, tr_vector),
                            np.dot(y_axis, tr_vector),
                            np.dot(z_axis, tr_vector)])

    f_tr_fwj = math.atan2(tr_vector_t[1], tr_vector_t[0])  # 方位角 horizon angle in XY plane, a vector from obj to robot
    f_tr_yj = math.atan2(tr_vector_t[2], tr_vector_t[0])  # 仰角   elevation angle in XZ plane
...
def body_collision(body1, body2, max_distance=MAX_DISTANCE, visualization=False):  # 10000
...
def collision_dist(body1, body2, max_distance=MAX_DISTANCE, visualization=False):  # 10000
    # TODO: confirm that this doesn't just check the base link
...
def get_collision_fn(robot, joints, obstacles, attachments, self_collisions, disabled_collisions,
    ...
    def collision_fn(q):
        """
        Return True if collision arises, when the robot is at configuration q.
        Collision can arise from:
        """
...
def check_initial_end(start_conf, end_conf, collision_fn):
    if collision_fn(start_conf):
        print("Warning: initial configuration is in collision. Path planning is impossible.")
        return False
    if collision_fn(end_conf):
        print("Warning: end configuration is in collision. Path planning is impossible.")
        return False
    return True
...
def sample_placement_on_aabb_seed(top_body, bottom_aabb, seed, top_pose=unit_pose(),
                                  percent=0.5, max_attempts=100, epsilon=1e-3):
    # TODO: transform into the coordinate system of the bottom
    # TODO: maybe I should instead just require that already in correct frame
    for _ in range(max_attempts):
        # theta = np.random.uniform(*CIRCULAR_LIMITS)
...
def sample_placement_on_aabb_collision(top_body, surface, bottom_aabb, all_bodies, top_pose=unit_pose(),
                                       percent=0.5, max_attempts=200, epsilon=1e-3):
...
class Attachment(object):
    def __init__(self, parent, parent_link, attach_pose, child):
        self.parent = parent
        self.parent_link = parent_link
        self.grasp_pose = attach_pose
        self.child = child
