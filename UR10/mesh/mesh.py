...
class icosphere(object):
    ...
    def pool_indices(self, g, l_prev):
        """
        return:
        [(target_v, neighbor_v1, ..., neighbor_v5),...] for base vertices,
        [(target_v, neighbor_v1, ..., neighbor_v5, neighbor_v6),...] for rest vertices.
        Indicates how pooling will be done.
        """
        ...
        """Neighbor set (size=5) of each base vertex (x12)"""
        ...
        if nv_rest:  # empty for L1 -> L0 pooling
            ...
            """Neighbor set (size=6) of each base vertex (x12)"""
...
def show_icoshere(icosphere_mesh):
    xs = []
    ys = []
    zs = []

    for v in icosphere_mesh.vertices:
        x, y, z = v
        xs.append(x)
        ys.append(y)
        zs.append(z)
